...
struct GoalCell: View {
    ...
    init(goal: GoalViewModel, showingEquivalentValues: Binding<Bool>) {
        if goal.isAutoGenerated {
            print("Creating GoalCell with: \(goal.debugDescription)")
        }
        self.goal = goal
...
//        }
    ...
    var texts: some View {
        var texts: some View {
            if goal.isAutoGenerated {
                print("   goal bounds are: \(goal.lowerBound), \(goal.upperBound)")
                print("   texts using: \(lowerBound), \(upperBound)")
            }
            return HStack {
                if let lowerBound {
                    if upperBound == nil {
